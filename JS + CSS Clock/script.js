...
function setDate() {
	...
	const month = new Intl.DateTimeFormat('pt-BR', { month: 'short'}).format(now).split('.')[0].toUpperCase();
	monthDiv01.innerText = month[0];
	monthDiv02.innerText = month[1];
	monthDiv03.innerText = month[2];
	
	const dayLetters = new Intl.DateTimeFormat('pt-BR', { weekday: 'short'}).format(now).split('.')[0].toUpperCase();
